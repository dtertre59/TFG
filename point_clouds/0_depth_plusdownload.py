...
def export_image(filename: str, frame):
    # Escribir el frame como una imagen PNG
...
with device:
    ...
    while True:
        ...
        if frameRgb is not None and frameDisp is not None and frameC is not None:
            ...
            if len(frameDisp.shape) < 3: # : Esta línea de código verifica si la longitud de la forma (shape) de la imagen frameDisp es menor que 3. Si es menor, significa que la imagen es en escala de grises, ya que las imágenes en escala de grises tienen solo 2 dimensiones (alto y ancho).
            ...
            if key == ord('q'):
                ...
                export_image(filename=str(image_directory / 'disparity_1.png'), frame=frameDisp)
                export_image(filename=str(image_directory / 'conf_1.png'), frame=frameC)
                export_image(filename=str(image_directory / 'rgb_1.png'), frame=frameRgb)
